...
export default function UpdateCustomers() {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:3001/getCustomers/' + id)
            .then(result => {
                if (result.data._id) {
                    ...
                    setLaptopName(result.data.LaptopName);
                    setLaptopBrand(result.data.LaptopBrand);
                    setLaptopModel(result.data.LaptopModel);
                    setActiveStatus(result.data.ActiveStatus);
                    setLaptopQty(result.data.LaptopQty);
    ...
    function check(resultcheck) {
        if (resultcheck == 'success') {
            ...
            setTimeout(() => { navigate('/CustomerInventories') }, 300);
        }
    ...
    const submit = (e) => {
        ...
        e.preventDefault();
        axios.put('http://localhost:3001/UpdateCustomers/' + id, { CustomerName, ContactNumber, Email, Address, LaptopName, LaptopBrand, LaptopModel, ActiveStatus, LaptopQty, CreatedAt, UpdatedAt })
            .then(result => {
            ...
            <div className="container-fluid update-laptop">
                <nav>
                    <ol className="breadcrumb">
                        ...
                        <li className="breadcrumb-item active" aria-current="page">Update Customer</li>
                    </ol>
                </nav>
                <form onSubmit={submit} className="mt-5 mb-5">

                    <h3 className="mb-4">Update Customers</h3>
                    ...
                    <div className={`alert alert-danger fade ${result3 ? 'd-block show' : 'd-none hide'}`} role="alert">
                        Please check <strong> {result3}</strong> can not be repeated, Please try again!
                    </div>
                    ...
                    <div className="row">
                        <div className="col-lg-3 col-md-3 col-6 p-2">
                            ...
                            {CustomerName}
                            <input type="text" defaultValue={result.CustomerName} onInput={(e) => setCustomerName(e.target.value)} className="form-control" id="exampleCustomer" required />
                        </div>
                        <div className="col-lg-3 col-md-3 col-6 p-2">
                            <label htmlFor="exampleContact" className="form-label " style={{ fontWeight: '600' }}>Contact No </label>
                            <input type="number" defaultValue={result.ContactNumber} onInput={(e) => setContactNumber(e.target.value)} className="form-control" id="exampleContact" required />
                        </div>
                        <div className="col-lg-3 col-md-3 col-6 p-2">
                            <label htmlFor="exampleEmail" className="form-label " style={{ fontWeight: '600' }}>Email  </label>
                            <input type="email" defaultValue={result.Email} onInput={(e) => setEmail(e.target.value)} className="form-control" id="exampleEmail" required />
                        </div>
                        <div className="col-lg-3 col-md-3 col-6 p-2">
                            <label htmlFor="exampleAddress" className="form-label " style={{ fontWeight: '600' }}>Address </label>
                            <input type="text" defaultValue={result.Address} onInput={(e) => setAddress(e.target.value)} className="form-control" id="exampleAddress" />
                        </div>
                        ...
                        <div className={`${Laptops[0].LaptopBrand ? 'd-block' : 'd-none'}col-lg-3 col-md-3 col-6 p-2`}>
                            <label htmlFor="exampLaptopBrand" className="form-label " style={{ fontWeight: '600' }}>Laptop Brand : {LaptopBrand}</label>
                            <select className="form-control" defaultValue={result.LaptopBrand} id="exampLaptopBrand" onInput={(e) => setLaptopBrand(e.target.value)} >
                        ...
                        <div className={`${Laptops[0].LaptopBrand ? 'd-block' : 'd-none'}col-lg-3 col-md-3 col-6 p-2`}>
                            <label htmlFor="exampLaptopName" className="form-label " style={{ fontWeight: '600' }}>Laptop Name : {LaptopName} </label>
                            <select className="form-control" defaultValue={result.LaptopName} id="exampLaptopName" onInput={(e) => setLaptopName(e.target.value)} >
                                <option value="" >Select</option>
                                {Laptops.map((laptop, key) => {
                                    return (
                        ...
                        <div className={`${Laptops[0].LaptopBrand ? 'd-block' : 'd-none'}col-lg-3 col-md-3 col-6 p-2`}>
                            <label htmlFor="exampLaptopModel" className="form-label " style={{ fontWeight: '600' }}>Laptop Model : {LaptopModel}</label>
                            <select className="form-control" defaultValue={result.LaptopModel} id="exampLaptopModel" onInput={(e) => setLaptopModel(e.target.value)} >
                                <option value="" >Select</option>
                                {Laptops.map((laptop, key) => {
                                    return (
                        ...
                        <div className={`${Laptops[0].LaptopBrand ? 'd-block' : 'd-none'}col-lg-3 col-md-3 col-6 p-2`}>
                            ...
                            <input type="number" defaultValue={result.LaptopQty} onInput={(e) => setLaptopQty(e.target.value)} className="form-control" id="exampleLaptopQty" />
                        </div>
